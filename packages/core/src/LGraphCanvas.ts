...
export interface GraphDialogOptions {
  position?: Vector2
  event?: MouseEvent
...
export interface ClipboardClonedNodeInfo {
  prevNodeID: NodeID
...
implements LGraphCanvas_Rendering, LGraphCanvas_UI, LGraphCanvas_Events {
  ...
  closeSubgraph(): void {
    ...
    const subgraph_node = (this.graph as any)._subgraph_node
  ...
  adjustCanvasForHiDPI(ratio?: number) {
    ...
    if (ratio === 1 || !this.canvas.parentNode)
      return
  ...
  createDefaultNodeForSlot(nodeType: NodeTypeSpec, opts: LCreateDefaultNodeForSlotOptions = {}): boolean { // addNodeMenu for connection
    ...
    let slotX: SlotNameOrIndex | INodeSlot = isFrom ? opts.slotFrom : opts.slotTo
    ...
    if (slotTypesDefault && fromSlotSpec) {
      ...
      if (Array.isArray(fromSlotSpec)) {
        for (const index in fromSlotSpec) {
      ...
      if (nodeNewType) {
        let nodeNewOpts: NodeTypeOpts | null = null
  ...
  copyToClipboard(): void {
    const clipboard_info: ClipboardInfo = {
      nodes: [],
      nodeCloneData: {},
      links: [],
    }
    ...
    for (let i = 0; i < selected_nodes_array.length; ++i) {
      ...
      const cloned = node.clone(cloneData)
      if (!cloned) {
  ...
  pasteFromClipboard(): void {
    ...
    // create nodes
    const clipboard_info = JSON.parse(data) as ClipboardInfo
    ...
    for (var i = 0; i < clipboard_info.nodes.length; ++i) {
      ...
      if (node) {
        ...
        this.graph.add(node, { doProcessChange: false, addedBy: 'paste', prevNodeID, cloneData })
      ...
      switch (w.type) {
        ...
        case 'text':
          if (event.type === `${LiteGraph.pointerevents_method}down`) {
            ...
            }.bind(w), event, w.options ? w.options.multiline : false, w.options.inputStyle)
          }
          break
        default:
          if (w.mouse)
