...
(function () {
  ...
  const MidiParser = {
     ...
     */
    addListener(_fileElement, _callback) {
      if (!File || !FileReader)
      ...
      _fileElement.addEventListener('change', (InputEvt) => { // set the 'file selected' event handler
        if (!InputEvt.target.files.length)
          return false // return false if no elements where selected
    ...
    Uint8(FileAsUint8Array) {
      ...
      const MIDI = {} // create new midi object
      ...
      for (let t = 1; t <= MIDI.tracks; t++) {
        ...
        if (headerValidation !== 0x4D54726B)
          return false // Track chunk header validation failed.
        ...
        while (!endOfTrack) {
          ...
          //
            ...
            MIDI.track[t - 1].event[e - 1].type = Number.parseInt(statusByte[0], 16)// first byte is EVENT TYPE ID
            MIDI.track[t - 1].event[e - 1].channel = Number.parseInt(statusByte[1], 16)// second byte is channel
  ...
  if (typeof module !== 'undefined') { module.exports = MidiParser }
    ...
    const _global = typeof window === 'object' && window.self === window && window
