...
!(function () {
  ...
  AudioSynth = function AudioSynth() {
    if (AudioSynthInstance instanceof AudioSynth) { return AudioSynthInstance }
  ...
  setPub('setVolume', function (v) {
    v = Number.parseFloat(v); if (isNaN(v))
      v = 0
  ...
  setPub('generate', function (sound, note, octave, duration) {
    ...
    const time = !duration ? 2 : Number.parseFloat(duration)
...
Synth.loadModulationFunction(
  (i, sampleRate, frequency, x) => { return 1 * Math.sin(2 * Math.PI * ((i / sampleRate) * frequency) + x) },
  (i, sampleRate, frequency, x) => { return 1 * Math.sin(4 * Math.PI * ((i / sampleRate) * frequency) + x) },
  (i, sampleRate, frequency, x) => { return 1 * Math.sin(8 * Math.PI * ((i / sampleRate) * frequency) + x) },
  (i, sampleRate, frequency, x) => { return 1 * Math.sin(0.5 * Math.PI * ((i / sampleRate) * frequency) + x) },
  (i, sampleRate, frequency, x) => { return 1 * Math.sin(0.25 * Math.PI * ((i / sampleRate) * frequency) + x) },
  (i, sampleRate, frequency, x) => { return 0.5 * Math.sin(2 * Math.PI * ((i / sampleRate) * frequency) + x) },
  (i, sampleRate, frequency, x) => { return 0.5 * Math.sin(4 * Math.PI * ((i / sampleRate) * frequency) + x) },
  (i, sampleRate, frequency, x) => { return 0.5 * Math.sin(8 * Math.PI * ((i / sampleRate) * frequency) + x) },
  (i, sampleRate, frequency, x) => { return 0.5 * Math.sin(0.5 * Math.PI * ((i / sampleRate) * frequency) + x) },
  (i, sampleRate, frequency, x) => { return 0.5 * Math.sin(0.25 * Math.PI * ((i / sampleRate) * frequency) + x) },
)
...
Synth.loadSoundProfile({
  ...
  wave(i, sampleRate, frequency) {
    ...
    if (valueTable.length <= Math.ceil(period)) {
      ...
      if (resetPlay)
        vars.playVal = 0; else vars.playVal++
